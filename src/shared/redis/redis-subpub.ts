...
export class RedisSubPub {
  ...
  constructor(
    ...
    private channelPrefix: string = "a1-math-club-channel#"
  ) {
    this.init();
  }
  ...
  public async publish(event: string, data: any) {
    ...
    if (event !== "log")
      Logger.debug(`Publish:${channel} <- ${_data}`, RedisSubPub.name);

    await this.pubClient.publish(channel, _data);
  }

  private ctc = new WeakMap<
    Function,
  ...
  public async subscribe(event: string, callback: (data: any) => void) {
    ...
    const cb = (channel: any, message: any) => {
      if (channel === myChannel) {
        if (event !== "log")
          Logger.debug(`Subcribe:${channel} -> ${message}`, RedisSubPub.name);
